...
def enable_z_pass(scene):
    # Enable Z-pass
    for view_layer in scene.view_layers:
        view_layer.use_pass_z = True


def setup_depth_output(scene):
    ...
    # Create render layer node and file output node
    rl_node = tree.nodes.new("CompositorNodeRLayers")
    file_output_node = tree.nodes.new("CompositorNodeOutputFile")

    # Ensure correct output slot connection
    tree.links.new(rl_node.outputs["Depth"], file_output_node.inputs[0])

    file_output_node.format.file_format = (
        "OPEN_EXR"  # Use OpenEXR format to save depth information
    )
    file_output_node.format.color_depth = "32"
    # file_output_node.format.color_mode = 'BW'


def extract_point_cloud_from_objects(objects, transformation_matrix=None):
    """
    Extracts the point cloud (vertices) from the given Blender object.

    Args:
        object (bpy.types.Object): The Blender object from which to extract the point cloud.
        transformation_matrix (np.array): Optional transformation matrix to apply to each point.

    Returns:
        numpy.ndarray: An array of points, each with x, y, z coordinates.
    """
    pt3d = []
    for object in objects:
        ...
        if transformation_matrix is not None:
            vertices = [
                transformation_matrix @ np.array(vertex.co.to_4d())
                for vertex in mesh.vertices
            ]
            # Convert homogeneous coordinates back to 3D
            vertices = np.array([v[:3] / v[3] for v in vertices])
        else:
            vertices = [vertex.co for vertex in mesh.vertices]

        pt3d.extend(vertices)

    print(f"Extracted {len(pt3d)} points from {len(objects)} objects.")
...
def render_object(
    ...
    enable_z_pass(scene)
    setup_depth_output(scene)

    for i in range(num_renders):
        # Set camera
        camera = randomize_camera(
            only_northern_hemisphere=only_northern_hemisphere, scene=scene
        )
        scene.camera = camera  # Ensure the camera is set to the scene

        # Render the image
        render_path = os.path.join(output_dir, "rgb", f"{i:03d}.png")
        scene.render.filepath = render_path

        # Update the file path for depth map in the same directory
        depth_base_path = os.path.join(output_dir, "depth")
        scene.node_tree.nodes["File Output"].base_path = depth_base_path
        depth_file_name = f"{i:03d}_"
        scene.node_tree.nodes["File Output"].file_slots[0].path = depth_file_name

        bpy.ops.render.render(write_still=True, animation=False)

        # Save camera RT matrix
        rt_matrix = get_3x4_RT_matrix_from_blender(camera)
        rt_matrix_path = os.path.join(output_dir, "camera_pose", f"{i:03d}.npy")
        os.makedirs(os.path.dirname(rt_matrix_path), exist_ok=True)
        np.save(rt_matrix_path, rt_matrix)

        # Save camera intrinsics
        cam = camera  # Assuming 'camera' is the camera object
        intrinsics = {
        ...
        fx = intrinsics["focal_length"] * (
            intrinsics["width"] / intrinsics["sensor_width"]
        )
        fy = intrinsics["focal_length"] * (
            intrinsics["height"] / intrinsics["sensor_height"]
        )
        ...
        intrinsics_path = os.path.join(output_dir, "intrinsics", f"{i:03d}.npy")
        os.makedirs(os.path.dirname(intrinsics_path), exist_ok=True)
        np.save(intrinsics_path, K)

    # extract point cloud from scene
    # print("Extracting point cloud from scene...")
    # pt3d = extract_point_cloud_from_objects(scene.objects)

    # pt3d_path = os.path.join(output_dir, "point_cloud/pt3d.npy")
    # os.makedirs(os.path.dirname(pt3d_path), exist_ok=True)
    # np.save(pt3d_path, pt3d)

    # save blender scene as glb

    # glb_path = os.path.join(output_dir, f"{object_uid}.glb")
    # bpy.ops.export_scene.gltf(filepath=glb_path, export_format="GLB", export_animations=False)
    # print(f"Saved GLB file to {glb_path}")

    # save .blender file
    blend_path = os.path.join(output_dir, f"{object_uid}.blend")
...
if __name__ == "__main__":
    ...
    scene.cycles.filter_width = 0.01  # 0.01
    ...
    # Set Eevee settings (Example; you may need to adjust based on your needs)
