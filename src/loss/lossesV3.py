...
class MultiLoss (nn.Module):
    ...
    def compute_single_loss(self, *args, **kwargs):
        raise NotImplementedError()
    
    def loss_select(self, *args, **kwargs):
        '''
            select single loss or normal loss based on args number
        '''
        if len(args) == 2:
            return self.compute_single_loss(*args, **kwargs)
        else: 
    ...
    def forward(self, *args, **kwargs):
        loss = self.loss_select(*args, **kwargs)
...
class Regr3D (Criterion, MultiLoss):
    ...
    def get_single_pts3d(self, gt, pred, dist_clip=None, ref=False):
        
        # in_camera1 = inv(gt['camera_pose'])
        # gt_pts = geotrf(in_camera1, gt['pts3d'])  # B,H,W,3
        
        gt_pts = gt['pts3d'].clone()
        ...
        pr_pts = get_pred_pts3d(gt, pred, use_pose=False)
        
        if self.norm_mode:
            pr_pts = normalize_pointcloud(pr_pts, None, self.norm_mode, valid, None)
        if self.norm_mode and not self.gt_scale:
            gt_pts = normalize_pointcloud(gt_pts, None, self.norm_mode, valid, None)
            
            
        return gt_pts, pr_pts, valid
    ...
    def compute_single_loss(self, gt, pred, **kw):
        
        gt_pts, pred_pts, valid = self.get_single_pts3d(gt, pred, **kw)
        
        if kw.get('ref', False):
            # use kd-tree to find the nearest GT point
            from scipy.spatial import cKDTree
            tree = cKDTree(gt_pts[valid].detach().cpu().numpy())
            _, idx = tree.query(pred_pts[valid].detach().cpu().numpy())
            l = self.criterion(pred_pts[valid], gt_pts[valid][idx])
        else:
            l = self.criterion(pred_pts[valid], gt_pts[valid])
        
        self_name = type(self).__name__
        details = {self_name+'_pts3d': float(l.mean())}
...
class ConfLoss (MultiLoss):
    ...
    def compute_single_loss(self, gt, pred, **kw):
        ...
        (loss, msk), details = self.pixel_loss(gt, pred, **kw)
        
        if kw.get('ref', False):
            return loss.mean(), dict(conf_loss=float(0), **details)
        
        if loss.numel() == 0:
            print('NO VALID POINTS in img', force=True)
        
        conf, log_conf = self.get_conf_log(pred['conf'][msk])
        
        conf_loss = loss * conf - self.alpha * log_conf
        
        conf_loss = conf_loss.mean() if conf_loss.numel() > 0 else 0
        
        return conf_loss, dict(conf_loss=float(conf_loss), **details)
...
class Regr3D_ShiftInv (Regr3D):
    ...
    def get_single_pts3d(self, gt, pred, dist_clip=None):
        
        gt_pts, pred_pts, valid = super().get_single_pts3d(gt, pred, dist_clip)
        
        gt_z = gt_pts[..., 2]
        pred_z = pred_pts[..., 2]
        
        gt_shift_z = get_joint_pointcloud_depth(gt_z, None, valid, None)[:, None, None]
        pred_shift_z = get_joint_pointcloud_depth(pred_z, None, valid, None)[:, None, None]
        
        gt_z -= gt_shift_z
        pred_z -= pred_shift_z
        
        return gt_pts, pred_pts, valid
...
class Regr3D_ScaleInv (Regr3D):
    ...
    def get_single_pts3d(self, gt, pred, dist_clip=None):
        ...
        _, gt_scale = get_joint_pointcloud_center_scale(gt_pts, None, valid, None)
        _, pred_scale = get_joint_pointcloud_center_scale(pred_pts, None, valid, None)
