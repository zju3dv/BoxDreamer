...
class DemoLoader(Dataset):
    def __init__(
        ...
        self.pose_filenames = glob.glob(os.path.join(SCENE_DIR, "poses/*.txt"))
        if sort_by_filename:
            self.pose_filenames = sorted(self.pose_filenames)
    ...
    def _prepare_batch(
        ...
        if self.relocalization_method == "align_gt":
            # make N*3*4 tensor:
            for i, pose_filename in enumerate(self.pose_filenames):
                # convert 4*4 to 3*4
