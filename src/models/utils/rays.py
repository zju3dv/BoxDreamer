...
from .normalize import intersect_skew_lines_high_dim
...
class Rays(object):
    ...
    def set_moments(self, moments):
        if self.is_plucker:
            ...
            self.to_plucker().set_moments(moments)
    
    def set_directions(self, directions):
        if self.is_plucker:
            self.rays[..., :3] = directions
        else:
            self.to_plucker().set_directions(directions)
...
def rays_to_cameras(
    ...
    R = torch.zeros_like(I_camera.R, dtype=torch.float32)
    for i in range(len(I_camera)):
        R[i] = compute_optimal_rotation_alignment(
            I_patch_rays[i].to(torch.float32),
            directions[i].to(torch.float32),
        )
...
def compute_optimal_rotation_alignment(A, B):
    ...
    H = B.T @ A
    U, _, Vh = torch.linalg.svd(H.to(torch.float32), full_matrices=True)
    s = torch.linalg.det((U.to(torch.float32) @ Vh.to(torch.float32)).to(torch.float32))
    S_prime = torch.diag(torch.tensor([1, 1, torch.sign(s)], device=A.device, dtype=torch.float32))
    return (U @ S_prime @ Vh).to(torch.float32)
