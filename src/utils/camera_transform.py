...
def adjust_camera_to_bbox_crop_(
    ...
    focal_length_px, principal_point_px = convert_ndc_to_pixels(
        fl, pp, image_size_wh
    )
    
    principal_point_px_cropped = principal_point_px - clamp_bbox_xywh[:2]

    focal_length, principal_point_cropped = convert_pixels_to_ndc(
...
def normalize_cameras(
    ...
    if compute_optical:
        new_cameras, points, scale = compute_optical_transform(
    ...
    if first_camera:
        new_cameras, points, coordinate_transform = first_camera_transform(new_cameras, points=points)
    else:
        coordinate_transform = torch.eye(4).to(new_cameras.R) # indentity matrix

    if normalize_trans:
        new_cameras, points, scale_3 = normalize_translation(
            new_cameras, points=points, max_norm=max_norm, query_idx=query_idx, first_camera=first_camera
        )
    else:
        scale_3 = 1.0
    
    
    return new_cameras, points, scale_3 * scale, coordinate_transform
...
def first_camera_transform(cameras: PerspectiveCameras, rotation_only=False, points=None):
    """
    ...
    return new_cameras, points, None


def normalize_translation(new_cameras, points=None, max_norm=False, query_idx=None, first_camera=True):
    
    t_gt = new_cameras.T.clone()
    assert query_idx is not None
    if query_idx is not None:
        # eliminate the query camera
        # use cameras except the query camera
        
        ref_idx = np.arange(len(new_cameras))
        ref_idx = ref_idx[ref_idx != query_idx]
        if first_camera and len(ref_idx) > 1:
            # eliminate first camera
            if 0 in ref_idx:
                ref_idx = ref_idx[ref_idx != 0]
        elif first_camera and len(ref_idx) == 1:
            # if only one camera as ref, do not eliminate the query camera
            ref_idx = np.arange(len(new_cameras))
        t_gt = t_gt[ref_idx]
